...
import { useChromeIdentity } from '../auth/ChromeIdentityProvider'
import '@/components/GmailWidget/GmailWidget.css'

// ─── Helpers (same as web version) ───────────────────────
...
export default function GmailWidget() {
  const { token, isReady, login, getToken } = useChromeIdentity()
  ...
  const fetchAll = useCallback(async (accessToken) => {
    ...
    try {
      ...
      // Token expired — try refreshing via chrome.identity
      if (listRes.status === 401 || listRes.status === 403) {
        try {
          const freshToken = await getToken(true) // forceRefresh=true
          return fetchAll(freshToken)
        } catch {
      ...
      if (!listRes.ok) {
        ...
        throw new Error(errBody?.error?.message || `HTTP ${listRes.status}`)
      }
  ...
  // Fetch on mount once chrome.identity has resolved
  useEffect(() => {
    if (isReady && token) fetchAll(token)
  }, [isReady, token, fetchAll])
  ...
  return (
    <div className="gmail-widget">
      ...
      <div className="gmail-list">
        {!token ? (
          <div className="gmail-state" onClick={handleLogin}>
